...
def classify(input, model, dustmodel=0, plot=1, useav=-99.0, ext=-99.0):
    ...
    # loglikelihoods
    if ((input.gmag > -99.0) & (input.rmag > -99.0)):
        #-(grcol-(mod['gmag'][um]-mod['rmag'][um]))**2./(2.*grcole**2.)
        lh_gr = loggaussian(grcol, mod['gmag'][um]-mod['rmag'][um], grcole)
    else:
        lh_gr = np.zeros(len(um))

    if ((input.rmag > -99.0) & (input.imag > -99.0)):
        lh_ri = loggaussian(ricol, mod['rmag'][um]-mod['imag'][um], ricole)
    else:
        lh_ri = np.zeros(len(um)) 

    if ((input.imag > -99.0) & (input.zmag > -99.0)):
        lh_iz = loggaussian(izcol, mod['imag'][um]-mod['zmag'][um], izcole)
    else:
        lh_iz = np.zeros(len(um)) 

    if ((input.jmag > -99.0) & (input.hmag > -99.0)):
        lh_jh = loggaussian(jhcol, mod['jmag'][um]-mod['hmag'][um], jhcole)
    else:
        lh_jh = np.zeros(len(um))

    if ((input.hmag > -99.0) & (input.kmag > -99.0)):
        lh_hk = loggaussian(hkcol, mod['hmag'][um]-mod['kmag'][um], hkcole)
    else:
        lh_hk = np.zeros(len(um))   

    if ((input.bmag > -99.0) & (input.vmag > -99.0)):
        lh_bv = loggaussian(bvcol, mod['bmag'][um]-mod['vmag'][um], bvcole)

    else:
        lh_bv = np.zeros(len(um))  

    if ((input.btmag > -99.0) & (input.vtmag > -99.0)):
        lh_bvt = loggaussian(bvtcol, mod['btmag'][um]-mod['vtmag'][um], bvtcole)

    else:
        lh_bvt = np.zeros(len(um))   

    if (input.teff > -99):
        lh_teff = loggaussian(input.teff, mod['teff'][um], input.teffe)
    else:
        lh_teff = np.zeros(len(um))

    if (input.lum > -99):
        #"lum" in mod.keys())
        lh_lum = loggaussian(input.lum, mod['lum'][um], input.lume)
    else:
        lh_lum = np.zeros(len(um))

    if (input.logg > -99.0):
        lh_logg = loggaussian(input.logg, mod['logg'][um], input.logge)
    else:
        lh_logg = np.zeros(len(um))

    if (input.feh > -99.0):
        lh_feh = loggaussian(input.feh, mod['feh'][um], input.fehe)
    ...
    if (input.plx > -99.0):
        ...
        #    lh_mabs =  -(mabs-mod_mabs[um])**2. / (2.*mabse**2.)
        #else:
        #    dv=mod_mabs[um]-mabs
        #    lh_mabs = np.log(10**(0.2*dv)) -((10**(0.2*dv)-1.)**2)/(2.*(input.plxe/input.plx)**2)
    else:
        lh_mabs = np.zeros(len(um))

    if (input.dnu > 0.):
        mod_dnu = dnusun*mod['fdnu']*np.sqrt(10**mod['rho'])
        lh_dnu =  -(input.dnu-mod_dnu[um])**2.0 / (2.0*input.dnue**2.0)
    else:
    ...
    if (input.numax > 0.):
        ...
        lh_numax = loggaussian(input.numax,mod_numax[um],input.numaxe)
    else:
    ...
    tllh = np.sum([lh_gr,lh_ri,lh_iz,lh_jh,lh_hk,lh_bv,lh_bvt,lh_teff,lh_logg,lh_feh,
                   lh_mabs,lh_dnu,lh_numax,lh_lum])
    ...
    if (input.plx > -99.0):
        lscale = 1350.
        dprior = np.log(mod['dis'][um]**2/(2.0*lscale**3.))-mod['dis'][um]/lscale
    else:
    ...
    for j in range(0,npar):
        try:
            ...
            setattr(result, names[j], 0.0)
            setattr(result, names[j]+'ep', 0.0)
            setattr(result, names[j]+'em', 0.0)
            setattr(result, names[j]+'px', 0.0)
            setattr(result, names[j]+'py', 0.0)
...
def reddening(model,um,avs,extfactors):
    ...
    model2={k:model[k][um] for k in model.keys()}
    ...
    model3 = {key:np.zeros(nmodels) for key in model}
